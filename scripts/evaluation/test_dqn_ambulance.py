"""
Evaluation script for DQN-Ambulance models trained with
train_project1_dqn.py.

Mirrors test_mappo_ambulance.py but uses DQNAgent:
  ...
  - Evaluation is always greedy (epsilon = 0).
"""
...
# Must be set before importing torch so CUDA is not initialised.
# DQN evaluation runs on CPU; this also keeps PARL from moving the model to GPU.
...
def run_test_episode(env, agent, device, verbose=False):
    """Run one greedy test episode; return a dict of metrics."""
    obs = env.reset()
    ...
    while True:
        ...
        # Greedy action selection (epsilon = 0 during evaluation)
        action_dict = {}
        for aid in obs.keys():
            obs_tensor = torch.FloatTensor(obs[aid]).unsqueeze(0).to(device)
    ...
    if stat_steps > 0:
        ...
        reward_stats = {
            'reg_waiting_mean': 0.0, 'reg_waiting_std': 0.0,
    ...
    avg_reward             = float(np.mean([total_reward[aid] for aid in agent_ids]))
...
def test_model(
    ...
    K / Z resolution order:
        1. CLI arguments (--K / --Z)
        2. exp_config.json next to the model file
        3. dqn_ambulance.yaml  ambulance.K / ambulance.Z
        4. Hard-coded defaults (0.5 / 3.0)
    """
    print("=" * 80)
    ...
    # ------------------------------------------------------------------
    # Resolve K and Z
    # K/Z priority: CLI > exp_config.json > YAML > hardcoded default
    # ------------------------------------------------------------------
    exp_K, exp_Z = None, None
    for candidate in [
        ...
        if os.path.exists(candidate):
            try:
                ...
                exp_K = exp_cfg.get('K')
                exp_Z = exp_cfg.get('Z')
                print(f"Loaded K={exp_K}, Z={exp_Z} from {candidate}")
            except Exception:
                pass
            break
    ...
    print(f"Using K={K}, Z={Z}")
    print(f"Reward formula : 50 - [(reg_mean + {K}*reg_std) + {Z}*(emg_mean + {K}*emg_std)]\n")
    ...
    # Build SUMO config
    # ------------------------------------------------------------------
    configs_dir = "tmp/test_dqn_ambulance_configs"
    ...
    # ------------------------------------------------------------------
    # Environment config
    # algorithm_name MUST be "project1_std_dqn" to match training
    # ------------------------------------------------------------------
    env_config = {
        ...
        "algorithm_name":        "project1_std_dqn",   # activates 68-dim obs + std reward
    ...
    # Explicitly move to CPU for evaluation regardless of what PARL/CUDA may have done.
    agent.alg.model.cpu()
    agent.alg.model.eval()
    device = torch.device('cpu')

    saved_eps = checkpoint.get('epsilon', 'N/A')
    saved_ep  = checkpoint.get('episode', 'N/A')
    print(f"Model loaded successfully.")
    print(f"   Saved at episode : {saved_ep}")
    print(f"   Saved epsilon    : {saved_eps}")
    ...
    for ep in range(num_episodes):
        ...
        result = run_test_episode(env, agent, device=device, verbose=verbose)
    ...
    summary = {
        ...
        'reg_waiting_mean_mean': reg_mean_mean,
        'reg_waiting_mean_std':  reg_mean_std,
        'reg_waiting_std_mean':  reg_std_mean,
        'reg_waiting_std_std':   reg_std_std,
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate a trained DQN-Ambulance model',
