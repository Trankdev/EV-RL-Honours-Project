...
class World(parse_sumo_config, gym.Env):
    ...
    def __init__(self, sumo_config, **kwargs):
       ...
       # Distinguish between world-level and intersection-level metrics       
        ...
        # NEW FOR LEXICOGRAPHIC MORL: system-wide r1 (EV) signal, computed
        # once per sub-step in step_sim_and_statistics() and read identically
        # by every intersection's GetRewards (see Rewards.py
        # _lex_objective_ev_priority). 0.0 = neutral (no EV currently
        # delayed anywhere), NOT a penalty - this is what keeps the EV
        # objective well-behaved when no EV is present.
        self._lex_shared_r1_ev = 0.0
        self._lex_ambulance_type_ids = ['ambulance_type', 'emergency']
    ...
    # =====================================
    def _compute_shared_lex_ev_signal(self) -> float:
        """
        NEW FOR LEXICOGRAPHIC MORL.
        System-wide r1: negative sum of per-step timeloss accrued this
        sub-step by every EV currently in the network, wherever it is.
        Symmetric with Rewards.py's r2 (which does the same sum, filtered
        to regular vehicles, but only over one intersection's own lanes).
        0.0 whenever there is no EV in the network - by construction, NOT
        a penalty, which is what keeps the lexicographic threshold logic
        in MAPPOagent.py well-behaved during EV-free stretches.
        """
        total_ev_delay = 0.0
        try:
            for veh_id in self.eng.vehicle.getIDList():
                try:
                    if self.eng.vehicle.getTypeID(veh_id) in self._lex_ambulance_type_ids:
                        total_ev_delay += self.vehicle_timeloss_delta.get(veh_id, 0.0)
                except Exception:
                    continue
        except Exception:
            pass
        return -float(total_ev_delay)
    ...
    def step_sim_and_statistics(self): # actually is statistic first and then step sim
        ...
        # NEW FOR LEXICOGRAPHIC MORL: refresh the system-wide r1 (EV) signal
        # now that vehicle_timeloss_delta is up to date for this sub-step.
        # Every intersection's next accumulate_reward() call (which, like the
        # existing reward functions, reads vehicle_timeloss_delta with the
        # same one-sub-step lag already present elsewhere in this file) will
        # see this same broadcast value.
        self._lex_shared_r1_ev = self._compute_shared_lex_ev_signal()
    ...
    def reset(self):
        ...
        self._lex_shared_r1_ev = 0.0  # NEW FOR LEXICOGRAPHIC MORL: reset each episode
