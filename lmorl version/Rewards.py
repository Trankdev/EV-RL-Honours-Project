...
class GetRewards(ObservationFunction):
    ...
    # is priority level 1. r1 is a SYSTEM-WIDE, broadcast value — it is
    # computed once per simulation sub-step at World level (see
    # World.step_sim_and_statistics -> World._compute_shared_lex_ev_signal)
    # and every intersection reads the same cached number, since a single
    # EV's progress is a corridor-level property, not a local one. r2 stays
    # local to this intersection's own lanes, same as the existing reward
    # functions.
    #
    # TO ADD/CHANGE AN OBJECTIVE: write a `_lex_objective_<name>` method
    # that returns a float, then add/reorder its name in
    # LEXICOGRAPHIC_OBJECTIVES below. Index 0 = highest priority. Nothing
    # else needs to change — MAPPOagent.py reads n_objectives off the
    # length of the vector this returns.
    ...
    LEXICOGRAPHIC_CONFIG = {
        ...
        'max_tta': 60.0,        # seconds, for normalising EV time-to-arrival urgency
        'max_weight': 75.0,     # cap on the TTA-derived urgency weight (prevents explosion)
        'div_error_avoider': 1e-4,
    }

    # TODO: LEXICOGRAPHIC functions and configs around here

    def _lex_objective_ev_priority(self) -> float:
        """
        r1: system-wide EV objective. Reads the value World computed once
        this sub-step (identical for every agent) - see env.py
        World._compute_shared_lex_ev_signal(). Falls back to 0.0 (neutral,
        NOT a penalty) if World hasn't been updated to compute it yet, so
        this degrades safely rather than crashing.
        """
        return float(getattr(self.world, '_lex_shared_r1_ev', 0.0))

    def _lex_objective_regular_vehicles(self) -> float:
        """
        r2: local regular-vehicle objective for THIS intersection only.
        Negative sum of per-vehicle timeloss accrued this sub-step across
        this intersection's observed lanes, excluding EVs. Group1/group2
        are NOT split here (that split remains available for diagnostics
        via get_reward_statistics(), it's just not used to gate training).
        """
                ...
                for veh_id in vehicle_ids:
                    try:
                        ...
                        if veh_type in ambulance_type_ids:
                            continue
                        total_delay += self.world.vehicle_timeloss_delta.get(veh_id, 0.0)
                    except Exception:
                        continue
